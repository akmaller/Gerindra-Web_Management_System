    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <a href="{{ route('home') }}" class="flex items-center">
                @if($settings?->logo_path)
                    <img src="{{ asset('storage/'.$settings->logo_path) }}" alt="{{ $settings->site_name }}" class="h-12 w-auto" width="160" height="40" fetchpriority="auto">
                @else
        ...
        <div class="flex items-center gap-4">
            ...
            @if(!empty($profile))
                <div class="hidden lg:flex items-center gap-3 text-[color:var(--brand-primary)] opacity-70">
            ...
            <form action="{{ route('search') }}" method="get" class="hidden lg:block">
                <input
                    ...
                    aria-label="Cari berita atau halaman" maxlength="200"
        ...
        <button
            @click="open = !open"
            type="button" aria-controls="mobile-navigation"
            :aria-expanded="open"
            :aria-label="open ? 'Tutup menu' : 'Buka menu'"
            class="lg:hidden inline-flex items-center justify-center rounded-lg p-2 ring-1 ring-neutral-300
                transition-colors"
...
<div
    ...
    x-collapse
    x-cloak
    class="lg:hidden w-full bg-white border-t border-neutral-200"
    @keydown.window.escape="open=false"
>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        <form action="{{ route('search') }}" method="get" role="search" class="flex gap-2">
            <label for="mobile-search" class="sr-only">Cari berita atau halaman</label>
            <input id="mobile-search" name="q" type="search" maxlength="200" placeholder="Cari berita atau halaman…" class="min-w-0 flex-1 rounded-lg border border-neutral-300 px-3 py-3 text-sm">
            <button type="submit" class="rounded-lg bg-[color:var(--brand-primary)] px-4 py-3 text-sm font-semibold text-white">Cari</button>
        </form>
